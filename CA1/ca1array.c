...
int main(){
    ...
    const bool  doIO         = true;     // determines whether or not the program outputs its results to a .dat file
    ...
    float dTdt;  // change in temperature over a small time step
    ...
    for (h=0; h<maxTimeSteps; h++){
        #pragma omp parallel for default(none) shared(T) private(i, j, dTdt) schedule(guided)
        for (i=1; i<501; i++){
            for (j=1; j<501; j++){
